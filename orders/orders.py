...
def create_order(cursor, body):
    ...
    cursor.execute("INSERT into public.\"Invoice\" (\"CustomerID\", \"LogtagInvoiceNumber\", \"Currency\", \"PurchaseOrderID\",\"CourierAccountID\",\"HSCodeID\", \"ShippingAddressID\" )  VALUES ( %s, %s, %s, %s, %s, %s ) RETURNING \"InvoiceID\"", ( str(int(body['CustomerID'])), logtagInvoiceNumber, body['Currency'], body['PurchaseOrderNumber'], str(int(body['CourierAccountID'])), str(int(body['HSCodeID'])), str(int(body['ShippingAddressID']))))
    row = cursor.fetchone()
